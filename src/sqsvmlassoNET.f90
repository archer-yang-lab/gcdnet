...
! USAGE:
!
! CALL sqsvmlassoNET(lam2, nobs, nvars, x, y, jd, pf, pf2, dfmax, pmax, nlam,&
...
SUBROUTINE sqsvmlassoNET (lam2, nobs, nvars, x, y, jd, pf, pf2, dfmax, pmax,&
  ...
  CALL sqsvmlassoNETpath(lam2, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax, pmax&
...
! ---------------------------------------------------------------------------- !
SUBROUTINE sqsvmlassoNETpath(lam2, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax,&
     ...
     ELSE
        ...
        ELSE IF (l == 2) THEN
           ...
           DO j = 1, nvars
              IF (ju(j) /= 0) THEN
                 IF (pf(j) > 0.0D0) THEN
                    al = MAX(al, ABS(DOT_PRODUCT(y * dl, x(:, j))) / pf(j))
                 END IF
              END IF
           END DO
           al = al * alf / nobs
